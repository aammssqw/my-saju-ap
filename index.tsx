...
const App = () => {
  ...
  const handleAnalyze = async (menu: MenuType) => {
    ...
    // 분석 시 마스터 모드는 초기화하여 손님용 UI(블러)를 먼저 보여줌
    setIsUnlocked(false); 
    setUnlockCode('');
    ...
    <div className="max-w-5xl mx-auto px-4 py-16 min-h-screen flex flex-col items-center">
          ...
          <div className="glass-panel p-6 md:p-10 rounded-[3rem] border-white/5 shadow-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
              <div className="space-y-3">
                <label className="text-[11px] text-zinc-500 font-black uppercase tracking-widest block text-left">생년월일</label>
                <input type="date" value={formData.date} onChange={e=>setFormData({...formData, date:e.target.value})} className="w-full bg-white/5 border border-white/10 p-5 rounded-2xl text-white outline-none focus:border-amber-500 transition-all text-center md:text-left"/>
              </div>
              <div className="space-y-3">
                <label className="text-[11px] text-zinc-500 font-black uppercase tracking-widest block text-left">태어난 시간</label>
                <input type="time" value={formData.time} onChange={e=>setFormData({...formData, time:e.target.value})} className="w-full bg-white/5 border border-white/10 p-5 rounded-2xl text-white outline-none focus:border-amber-500 transition-all text-center md:text-left"/>
              </div>
              <div className="space-y-3">
                <label className="text-[11px] text-zinc-500 font-black uppercase tracking-widest block text-left">성별</label>
          ...
          <div className="glass-panel w-full max-w-lg p-12 rounded-[3.5rem] border-amber-500/20 space-y-8">
            ...
            {currentMenu === 'COMPATIBILITY' ? (
              <div className="space-y-6">
                <input type="date" value={formData.partnerDate} onChange={e=>setFormData({...formData, partnerDate:e.target.value})} className="w-full bg-white/5 border border-white/10 p-5 rounded-2xl text-white outline-none text-center"/>
                <input type="time" value={formData.partnerTime} onChange={e=>setFormData({...formData, partnerTime:e.target.value})} className="w-full bg-white/5 border border-white/10 p-5 rounded-2xl text-white outline-none text-center"/>
                <div className="flex gap-3">
              ...
              <input type="date" value={formData.targetDate} onChange={e=>setFormData({...formData, targetDate:e.target.value})} className="w-full bg-white/5 border border-white/10 p-5 rounded-2xl text-white outline-none text-center"/>
            )}
            ...
            <div className="p-16 space-y-24 relative">
              {sajuResult.sections.map((sec: any, idx: number) => (
                <div key={idx} className={`space-y-10 ${!isUnlocked && idx > 0 ? 'filter blur-2xl opacity-10 select-none pointer-events-none' : ''}`}>
                  <h4 className="text-amber-500 text-4xl font-serif font-black tracking-[0.2em] uppercase flex items-center gap-6">
      ...
      <footer className="mt-48 w-full border-t border-white/5 pt-20 text-center space-y-20 no-print opacity-30 hover:opacity-100 transition-opacity">
        <div className="max-w-xs mx-auto space-y-6">
           {/* autoComplete="new-password" 추가로 브라우저 자동 채우기 방지 */}
           <input 
             type="password" 
           ...
           <button 
             ...
             className="text-[10px] text-zinc-800 font-black uppercase tracking-widest hover:text-amber-500 transition-all"
           >
             Master Unlock
           </button>
        </div>
